   ...
   <?php
	...
	if (isset($_POST['addbook'])) {
		...
		if ($inputError == false) {
            ...
            $query_insert_book = "INSERT INTO books (ID, Title, Category, ISBN, Price) ";
            $query_insert_book .= "VALUES (\"$bookid\", \"$title\", \"$category\", \"$isbn\", \"$price\")";
            if (!($result = $pdo->query($query_insert_book))) {
                print( "<p>Could not execute query!</p>" );
                die("</body></html>" );
            }
                echo "<p><strong>Table books was successfully updated.</strong></p>";


			//Write your code below to insert into the books-authors table as well:			
            foreach($author_arr as $author) {
                $query_insert_book_author = "INSERT INTO books_authors (BID, AID) ";
                $query_insert_book_author .= "VALUES (\"$bookid\", \"$authors_ids[$author]\")";
                if (!($result = $pdo->query($query_insert_book_author))) {
                    print( "<p>Could not execute query!</p>" );
                    die("</body></html>" );
                }
            }
			...
			//Display a message indicating that the table is successfully updated:
            echo "<p><strong>Table books and books_authors were successfully updated.</strong></p>";
			...
			$displayForm = false;
			echo "<p><a href=\"BooksDBInsertAllOne.php\">Add some more?</a></p>\n";
		}
	}
	//===========================================================================
	if ($displayForm) {
